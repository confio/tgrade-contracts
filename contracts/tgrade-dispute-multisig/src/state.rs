...
/// Non-mutable config initialized on contract instantiation
#[cw_serde]
pub struct Config {
    /// Address which instantiated this contract - it should be a contract which can handle the
    /// `RenderDecision` message
    pub parent: Addr,
    /// Votes threshold for voting to pass
    pub threshold: Threshold,
    /// Total weights points voting
    pub total_weight: u64,
    /// This voting expiration time
    pub expires: Expiration,
    /// Complaint id voted on, to be passed back to parent contract in `RenderDecision` message
...
impl State {
    ...
    pub fn current_status(&self, block: &BlockInfo, cfg: &Config) -> Status {
        let mut status = self.status;

        // if open, check if voting is passed or timed out
        if status == Status::Open && self.is_passed(block, cfg) {
            status = Status::Passed;
        }
        if status == Status::Open && (self.is_rejected(block, cfg) || cfg.expires.is_expired(block))
        {
    ...
    pub fn update_status(&mut self, block: &BlockInfo, cfg: &Config) {
        self.status = self.current_status(block, cfg);
    }
    ...
    pub fn is_passed(&self, block: &BlockInfo, cfg: &Config) -> bool {
        match cfg.threshold {
            ...
            Threshold::AbsolutePercentage {
                ...
                self.votes.yes
                    >= votes_needed(cfg.total_weight - self.votes.abstain, percentage_needed)
            }
            Threshold::ThresholdQuorum { threshold, quorum } => {
                // we always require the quorum
                if self.votes.total() < votes_needed(cfg.total_weight, quorum) {
                    return false;
                }
                if cfg.expires.is_expired(block) {
                    ...
                    let possible_opinions = cfg.total_weight - self.votes.abstain;
    ...
    pub fn is_rejected(&self, block: &BlockInfo, cfg: &Config) -> bool {
        match cfg.threshold {
            Threshold::AbsoluteCount {
                weight: weight_needed,
            } => {
                let weight = cfg.total_weight - weight_needed;
            ...
            Threshold::AbsolutePercentage {
                ...
                self.votes.no
                    > votes_needed(
                        cfg.total_weight - self.votes.abstain,
...
//    pub expires: Expiration,
//    pub msgs: Vec<CosmosMsg<TgradeMsg>>,
//    pub status: Status,
//    /// pass requirements
//    pub threshold: Threshold,
...
mod test {
    ...
    fn check_is_passed(
        ...
        let (cfg, state, block) = setup_state(threshold, votes, total_weight, is_expired);
        state.is_passed(&block, &cfg)
    }
