...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::Propose {
            ...
            description,
            proposal,
        } => execute_propose::<OversightProposal, Config>(
            deps,
            env,
            info,
            title,
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::Rules {} => to_binary(&query_rules::<Config>(deps)?),
        QueryMsg::Proposal { proposal_id } => to_binary(&query_proposal::<OversightProposal>(
            deps,
            env,
            proposal_id,
        )?),
        ...
        QueryMsg::ListProposals { start_after, limit } => {
            to_binary(&list_proposals::<OversightProposal>(
                deps,
                env,
                start_after,
                align_limit(limit),
        ...
        QueryMsg::Voter { address } => to_binary(&query_voter::<Config>(deps, address)?),
        QueryMsg::ListVoters { start_after, limit } => {
            to_binary(&list_voters::<Config>(deps, start_after, limit)?)
        }
    }
}
